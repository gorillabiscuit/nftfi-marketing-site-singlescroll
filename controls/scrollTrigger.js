...
export function setupSection4PebbleFadePinned(pebbleGroup1, pebbleGroup2, pebbleGroup3, pebbleGroup4) {
    ...
    if (bp === BREAKPOINT_NAMES.MOBILE) {
        ...
        try {
            ...
            // Ensure pebbles are visible and scaled/positioned from config
            ...
            // Apply scales
            gsap.set(pebbleGroup1.scale, { x: pcfg1.scale ?? 1, y: pcfg1.scale ?? 1, z: pcfg1.scale ?? 1 });
            if (pebbleGroup2) gsap.set(pebbleGroup2.scale, { x: pcfg2.scale ?? 1, y: pcfg2.scale ?? 1, z: pcfg2.scale ?? 1 });
            if (pebbleGroup3) gsap.set(pebbleGroup3.scale, { x: pcfg3.scale ?? 1, y: pcfg3.scale ?? 1, z: pcfg3.scale ?? 1 });
            if (pebbleGroup4) gsap.set(pebbleGroup4.scale, { x: pcfg4.scale ?? 1, y: pcfg4.scale ?? 1, z: pcfg4.scale ?? 1 });

            // Apply positions (no scroll offset)
            gsap.set(pebbleGroup1.position, { x: calculatePebbleX(pcfg1.position?.x ?? -2.5), y: (pcfg1.position?.y ?? -4.0), z: pcfg1.position?.z ?? 0 });
            if (pebbleGroup2) gsap.set(pebbleGroup2.position, { x: calculatePebbleX(pcfg2.position?.x ?? 2.5), y: (pcfg2.position?.y ?? -4.0), z: pcfg2.position?.z ?? 0 });
            if (pebbleGroup3) gsap.set(pebbleGroup3.position, { x: calculatePebbleX(pcfg3.position?.x ?? -2.5), y: (pcfg3.position?.y ?? -8.0), z: pcfg3.position?.z ?? 0 });
            if (pebbleGroup4) gsap.set(pebbleGroup4.position, { x: calculatePebbleX(pcfg4.position?.x ?? 2.5), y: (pcfg4.position?.y ?? -12.0), z: pcfg4.position?.z ?? 0 });

            // Make materials fully visible
            try { materials.forEach((m) => { try { m.transparent = true; m.opacity = 1; } catch (_) { /* no-op */ } }); } catch (_) { /* no-op */ }

            // Position panels relative to pebbles using a single projection pass
            const container = document.querySelector('.section4-content');
            if (container) {
                const containerRect = container.getBoundingClientRect();
                ...
                const projectPebbleToScreen = (pebbleGroup) => {
                    if (!pebbleGroup || !window.camera || !THREE) return null;
                ...
                const getPanelYPixels = (index, progress) => {
                    // Reuse vertical spacing logic for stable stacked layout
                    const titleOffset = typeof params.titleYOffset === 'number' ? params.titleYOffset : -400;
                    const spacing = typeof params.textSpacingPixels === 'number' ? params.textSpacingPixels : 300;
                    if (index === -1) return Math.round(window.innerHeight * 0.25 + titleOffset);
                    return Math.round(window.innerHeight * 0.25 + titleOffset + spacing * (index + 1));
                };

                // derive offsets and sizing
                const offsetLeft = (typeof params.textHorizontalOffsetLeft === 'number') ? params.textHorizontalOffsetLeft
                    : (typeof params.textHorizontalOffset === 'number' ? params.textHorizontalOffset : 0.9);
                const offsetRight = (typeof params.textHorizontalOffsetRight === 'number') ? params.textHorizontalOffsetRight
                    : (typeof params.textHorizontalOffset === 'number' ? params.textHorizontalOffset : 1.2);
                ...
                const applyPanel = (panelEl, pebbleGroup, index, side) => {
                    if (!panelEl || !pebbleGroup) return;
                    const yPos = getPanelYPixels(index, 1);
                    ...
                    if (side === 'right') {
                        ...
                        const availableRight = Math.max(0, rightEdge - pebbleScreen.x - half - gutter);
                        const desiredWidth = Math.min(Math.max(availableRight * 2, minW), maxW);
                        const xPosViewport = pebbleScreen.x + availableRight * offsetRight;
                        const xPos = clampToContainer(xPosViewport, panelEl, desiredWidth);
                        gsap.set(panelEl, { x: Math.round(xPos), y: Math.round(yPos), width: desiredWidth, xPercent: -50, yPercent: -50, opacity: 1 });
                    } else {
                        const leftEdge = containerLeft;
                        const availableLeft = Math.max(0, pebbleScreen.x - leftEdge - half - gutter);
                        const desiredWidth = Math.min(Math.max(availableLeft * 2, minW), maxW);
                        const xPosViewport = pebbleScreen.x - availableLeft * offsetLeft;
                        const xPos = clampToContainer(xPosViewport, panelEl, desiredWidth);
                        gsap.set(panelEl, { x: Math.round(xPos), y: Math.round(yPos), width: desiredWidth, xPercent: -50, yPercent: -50, opacity: 1 });
                    }
                };

                applyPanel(panel0, pebbleGroup1, 0, 'right');
                applyPanel(panel1, pebbleGroup2, 1, 'left');
                applyPanel(panel2, pebbleGroup3, 2, 'right');
                applyPanel(panel3, pebbleGroup4, 3, 'left');
            }
        } catch (_) { /* no-op */ }
