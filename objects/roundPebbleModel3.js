...
// Import RWA video for third pebble
import videoUrl from '../images/RWA.mp4?url';

export let roundPebbleMesh3 = null;
export let roundPebbleGroup3 = null;

/**
 * Load the third round pebble GLB with RWA video, and attach to the given parent group.
 * If parentGroup is provided (expected: pebbleGroup3), the round pebble inherits its transforms.
 */
export function loadRoundPebbleModel3(parentGroup, scene) {
    ...
    loader.load(roundPebbleUrl, (gltf) => {
        ...
        roundPebbleGroup3 = new THREE.Group();
        
        // Debug: Log the bounding box info
        console.log('[RoundPebble3] Bounding box info:', {
            center: center,
        ...
        roundPebbleGroup3.add(gltf.scene);
        roundPebbleGroup3.position.set(0, 0, 0);
        // Remove group rotation since we're rotating the plane directly
        roundPebbleGroup3.scale.setScalar(1);
        roundPebbleGroup3.visible = true;
        
        console.log('[RoundPebble3] Final plane position within pebble:', {
            scenePosition: gltf.scene.position,
            groupPosition: roundPebbleGroup3.position
        });

        if (parentGroup && parentGroup.add) {
            parentGroup.add(roundPebbleGroup3);
        } else if (scene && scene.add) {
            // Fallback: add to scene if parentGroup not available
            scene.add(roundPebbleGroup3);
        }

        // Expose for debugging
        window.ROUND_PEBBLE3 = { roundPebbleGroup3, roundPebbleMesh3, gltf: gltf.scene };
